...
public class TokenUtil {
    ...
    public Optional<Authentication> verifyToken(final HttpServletRequest request) throws UnsupportedEncodingException {
        ...
        if (decoded != "" && decoded.length() > 30) {// decoded.length() > 30 to ignore the static "access_token" text
                                                     // in swagger UI
    ...
    public String createTokenForUser(final User user) {
        return Jwts.builder().setExpiration(new Date(System.currentTimeMillis() + VALIDITY_TIME_MS)).setSubject(user.getEmail()).claim("id", user.getId())
                .claim("email", user.getEmail()).claim("role", user.getRole().toString()).claim("password", user.getPassword())
